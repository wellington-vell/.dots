{
  flake.modules.nixos.base =
    ...
    let
      ...
      # To use a rendered image avatar instead (kitty graphics in Ghostty,
      # chafa fallback elsewhere), change logo to:
      #   "type": "kitty",
      #   "source": "${pkgs.nixos-icons}/share/icons/hicolor/512x512/apps/nix-snowflake.png",
      #   "width": 22,
      #   "height": 11
      # Ghostty supports kitty; chafa is compiled in for fallback.
      ...
      # Image avatar variant (PNG snowflake via nixos-icons) for `fastfetch --kitty <path>`
      # Kept as a separate file for users who prefer an image over the ASCII logo.
    ...
    in
    {
      ...
      # System-wide fallback so every user gets the NixOS avatar even without activation.
      environment.etc."xdg/fastfetch/config.jsonc".source = fastfetchConfig;

      # Also expose image variant at /etc/xdg/fastfetch/config-image.jsonc
      # Run with: fastfetch --config /etc/xdg/fastfetch/config-image.jsonc
      environment.etc."xdg/fastfetch/config-image.jsonc".source = fastfetchImageConfig;

      # Per-user XDG config (fastfetch checks $XDG_CONFIG_HOME first).
